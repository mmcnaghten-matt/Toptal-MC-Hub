...
import { useSurvey, type CompletedRecord } from "@/context/SurveyContext";
import {
  surveyQuestions,
...
export const Route = createFileRoute("/admin")({
  head: () => ({
    meta: [{ title: "Admin — M&E Fan/Audience Diagnostic" }],
  }),
...
function AdminReportPage() {
  ...
  useEffect(() => {
    if (mounted && (!isAuthenticated || !isAdmin)) navigate({ to: "/" });
  ...
  const exportCSV = () => {
    ...
    a.href = url;
    a.download = `cannes-survey-results-${new Date().toISOString().slice(0, 10)}.csv`;
    a.click();
        ...
        <div className="mx-auto max-w-6xl px-6 py-8">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <h1 className="text-2xl font-bold md:text-3xl">Survey Reports</h1>
            </div>
            <button onClick={() => navigate({ to: "/" })} className="inline-flex items-center gap-2 rounded-lg border border-primary-foreground/30 px-4 py-2 text-sm text-primary-foreground hover:bg-primary-foreground/10 transition-colors">
