...
export const Route = createFileRoute("/thank-you")({
  head: () => ({
    meta: [{ title: "Thank You — M&E Fan/Audience Diagnostic" }],
  }),
  component: ThankYouPage,
});

function ThankYouPage() {
  const { resetSurvey, userInfo } = useSurvey();
          ...
          <div className="bg-card rounded-xl shadow-lg p-10 border border-border">
            ...
            {userInfo && (
              <p className="text-muted-foreground mb-2">
                <span className="font-medium text-foreground">{userInfo.name}</span> from{" "}
                <span className="font-medium text-foreground">{userInfo.company}</span>
              </p>
            )}

            <p className="text-muted-foreground leading-relaxed mb-8">
              Your responses have been recorded. Thank you for completing the M&E Fan/Audience Platform Diagnostic Assessment. Your insights will help inform strategic decisions for your platform journey.
            </p>
